...
apt_cache() {
	...
	do
		...
		case "$PATHNAME" in
			...
			*.debian.tar.gz|*.diff.gz|*.orig.tar.xz|*.orig.tar.gz|*.deb-control|*.dsc-cached) ;;
...
apt_cache_source() {
	...
	if [ ! -f "$VARLIB/apt/$DIST/$DIRNAME/$ORIG_FILENAME" ]; then
		ORIG_FILENAME="${NAME}_${ORIG_VERSION}.orig.tar.xz"
	fi
